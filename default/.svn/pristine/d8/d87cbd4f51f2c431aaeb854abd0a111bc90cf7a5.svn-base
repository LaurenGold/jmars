...
public class TriangleMesh implements GLRenderable, Disposable {
		...
		if (soid != null) {
			...
			try {
				createShapeMeshBuffers(gl);				
			        ...
			        log.aprint(ex);
			        JOptionPane.showMessageDialog(ThreeDManager.getInstance().getWindow(),
	...
	/* (non-Javadoc)
	 * @see edu.asu.jmars.viz3d.renderer.gl.GLAction#getId()
	 */
	@Override
	public int getId() {
	...
	private void realizeOcttree(boolean isObj) {
		...
		oct = new OctTree(root, max*2, min, /*max*/ maxLen, indices.length / 3);
		int j = 0;
		for (int i=0; i<indices.length; i+=3) {
	...
	public boolean rayIntersect (Ray ray, ArrayList<Triangle> tris) {
		if (oct != null) {
			...
			// SLD: Call a version of the octTreeRayIntersect that quits early if it detects a non-intersect
//			oct.qlooseOctTreeRayIntersect(ray, oct.getRoot(), tris);
		}
		if (tris.size() > 0) {
			return true;
		} else {
	...
	public void fitToMesh(OutLine outline) {
		if (!outline.isFitted() && outline.getOutLineType().equals(OutLineType.OnBody) 
				&& oct != null && outline.getPoints() != null 
				&& outline.getPoints().length > 5 /*need at least 6 values (3 points) to have a valid polygon */) {
			float[] pts = oct.looseFitOutLine(outline.getPoints());
			if (pts != null) {
