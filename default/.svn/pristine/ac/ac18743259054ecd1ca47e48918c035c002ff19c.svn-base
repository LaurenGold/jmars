...
//		mgr.setHighlightFacet(true);
	...
	//			 If we've already calculated these polygons once, use the ones we had before, but update the colors
				if (s.get3DPolygon()!=null) {
					...
					for (Polygon p : polygons) {
				        ...
				        if (stampLayer.spectraData() || stampLayer.lineShapes() || stampLayer.pointShapes()) {
				        	color = s.getCalculatedColor();
				        } else {
				        	color = myLView.getSettings().getFilledStampColor();        	
				        }
	
				        if (color!=null) {
							p.setFillColor(glColor(color));
							p.setColor(glColor(color));
						}
						p.setAlpha(getAlpha());
						fovs.add(p);
					}
				...
				//if its on earth or moon, we need to divide by the radius of the body because the shape mdoel is a unit sphere
				if(mgr.getShapeModel().isUnitSphere()){
					double bodyRad = Util.MEAN_RADIUS;
					
					if(bodyRad > 0){
						for(int i=0; i<intercepts.length; i++){
							intercepts[i].divEq(bodyRad);
						}
					}
				}
		        ...
		        if (color!=null) {
			        for(int j=0; j<segmentsOn.length; j++){
			            allPolygons[j]=
			            new Polygon(++id, 
			                    toFloat1D(reverse(segmentsOn[j])),
			                    glColor(color), glColor(color), 
			                    LINE_WIDTH, LINE_SCALE, 1.0f,
			                    getAlpha(),
			                    true, false);
