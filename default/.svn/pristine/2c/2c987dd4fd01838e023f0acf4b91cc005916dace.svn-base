...
public class StampShape {
	...
	private final int SPECTRA_TIP_OFFSET = 3;
	...
	private void calcTooltipIndex() {
		...
		//if this is a spectra layer, set the tip index to the calculated
		// value column
		if(stampLayer.spectraData()){
			tipIndex = myStamp.getData().length+SPECTRA_TIP_OFFSET;
			return;
		}
...
//		Analysis analysis = StampUtil.analyzePoints(myStamp.getPoints(), po);
		...
		if (analysis.is360wideSpatial()) {
    		BigDecimal flagVal=null;
    		int colNum = stampLayer.getColumnNum("pole_flag");
    		if (colNum!=-1) {
    			flagVal = (BigDecimal)getData(colNum);
    		}
...
//    			}
    ...
    public Point2D getBoresight() {
    	if (boresight==null && !boresightBad) {
	    	BigDecimal bx = (BigDecimal)getData(stampLayer.getColumnNum("boresight_intersection_x"));
	    	BigDecimal by = (BigDecimal)getData(stampLayer.getColumnNum("boresight_intersection_y"));
	    	BigDecimal bz = (BigDecimal)getData(stampLayer.getColumnNum("boresight_intersection_z"));
	    	
	    	if (bx==null || by==null || bz==null || bx.doubleValue()==0.0 || by.doubleValue()==0.0 || bz.doubleValue()==0.0) {
	    		BigDecimal blon = (BigDecimal)getData(stampLayer.getColumnNum("boresight_lon"));
	    		BigDecimal blat = (BigDecimal)getData(stampLayer.getColumnNum("boresight_lat"));
    ...
    {    
        ...
        if(paths == null)
        {
        	...
        	Analysis stats = analyzePoints(Main.PO);
