...
public class OutlineFocusPanel extends JPanel {
	...
	private JComboBox<String> orderColumnBx;
	private JCheckBox orderDirection;
	...
	public OutlineFocusPanel(final StampLayer stampLayer, final StampTable table, SpectraView spectra)
	{
		myLayer = stampLayer;
		myStampTable = table;
	...
	private void buildUI(){
        ...
        JButton exportBtn = new JButton(new AbstractAction("Export Table...".toUpperCase()) {
	    ...
	    /// NEW EXPORT        
	    JButton newExport = new JButton( new SaveAction("Export Table as CSV".toUpperCase(), ",")); 
	    /// NEW EXPORT
	    ...
	    //add to bottom panel
	    row = 0;
	    bot.add(countLbl, new GridBagConstraints(0, row, 3, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, in, pad, pad));
	    
	    if (myLayer.spectraData()) {
	    	row++;
	    	// TODO: Make these values save/restore
		    orderColumnBx = new JComboBox<String>();
		    orderColumnBx.addActionListener(orderColumnBoxListener);
		    //set a preferred size, and popup listener so the width can be shorter than the contents
		    orderColumnBx.setPreferredSize(new Dimension(150, orderColumnBx.getPreferredSize().height));
		    orderColumnBx.setMinimumSize(new Dimension(150, orderColumnBx.getPreferredSize().height));
		    orderColumnBx.addPopupMenuListener(cbListener);
		    orderDirection = new JCheckBox("Reverse draw order");
		    orderDirection.addActionListener(orderColumnBoxListener);
		    
   		    bot.add(new JLabel("Draw by:"), new GridBagConstraints(0, row, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, in, pad, pad));
		    bot.add(orderColumnBx, new GridBagConstraints(1, row, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.NONE, in, pad, pad));
		    bot.add(orderDirection, new GridBagConstraints(2, row, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.NONE, in, pad, pad));
	    }
	    	    
	    bot.add(limitToMainViewCBx, new GridBagConstraints(0, ++row, 2, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, in, pad, pad));

	    //if this is a spectra layer, add a button to view the spectra view
	    if(myLayer.spectraData()){
	    	JButton spectraBtn = new JButton(spectraAct);
	    	bot.add(spectraBtn, new GridBagConstraints(0, row+1, 3, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, in, pad, pad));
	    }
	    ...
	    if (myLayer.spectraData() || myLayer.pointShapes()) {
	    	...
	    	if(myLayer.spectraData()){
				...
				bot.add(hideOutofRangeCBx, new GridBagConstraints(2, row, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, in, pad, pad));
	    	}
	...
	//			text.setWrapStyleWord(true); //TODO: maybe not?
		    	...
		    	if ((mySettings.expressionText!=null && mySettings.expressionText.length()>0)|| mySettings.expressionSelected) {
		    ...
		    row = 0;
		    valuePnl.add(minValueLbl, new GridBagConstraints(1, row, 1, 1, 0, 0, GridBagConstraints.LINE_END, GridBagConstraints.NONE, in, pad, pad));
		    valuePnl.add(absMinTF, new GridBagConstraints(2, row, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.NONE, in, pad, pad));
		    valuePnl.add(maxValueLbl, new GridBagConstraints(3, row, 1, 1, 0, 0, GridBagConstraints.LINE_END, GridBagConstraints.NONE, in, pad, pad));
		    valuePnl.add(absMaxTF, new GridBagConstraints(4, row, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.NONE, in, pad, pad));
		    ...
		    row++;
		    valuePnl.add(new JLabel("Min value: "), new GridBagConstraints(1, row, 1, 1, 0, 0, GridBagConstraints.LINE_END, GridBagConstraints.NONE, in, pad, pad));
		    valuePnl.add(minTF, new GridBagConstraints(2, row, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.NONE, in, pad, pad));
		    valuePnl.add(new JLabel("Max value: "), new GridBagConstraints(3, row, 1, 1, 0, 0, GridBagConstraints.LINE_END, GridBagConstraints.NONE, in, pad, pad));
		    valuePnl.add(maxTF, new GridBagConstraints(4, row, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.NONE, in, pad, pad));
	    	row++;
	    	GridBagConstraints lockCons = new GridBagConstraints(0, row, 5, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, in, pad, pad);
	    	...
	    	if (myLayer.pointShapes() && !myLayer.fixedSpotSize()) {
	    		...
	    		valuePnl.add(new JLabel("Scale factor: "),new GridBagConstraints(3, row, 1, 1, 0, 0, GridBagConstraints.LINE_END, GridBagConstraints.NONE, in, pad, pad));
	    		valuePnl.add(scaleTF,new GridBagConstraints(4, row, 1, 1, 0, 0, GridBagConstraints.LINE_START, GridBagConstraints.HORIZONTAL, in, pad, pad));
	    	}
	...
	private ActionListener orderColumnBoxListener = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			myLayer.increaseStateId(StampLayer.OUTLINES_BUFFER);
			mySettings.orderColumn=(String)orderColumnBx.getSelectedItem();
			mySettings.orderDirection=orderDirection.isSelected();
			refreshViews();		
		}
	};
	...
	private void expressionChanged() {
		try {
			...
			exp = Evaluator.compile("convertReturnType("+alteredText+")", lib, Object.class);
	...
	public void setColorExpression(String text) {
		...
		int numCols = columnBx.getModel().getSize();
		
		for (int i=0; i<numCols; i++) {
			if (columnBx.getModel().getElementAt(i).equalsIgnoreCase(text)) {
				columnRBtn.doClick();
				columnBx.setSelectedIndex(i);
				expTA.setText("");
				errorsLbl.setText("");
				return;
			}
	...
	public void setOrderOption(String column, boolean reverse) {
		...
		int numCols = orderColumnBx.getModel().getSize();
		
		for (int i=0; i<numCols; i++) {
			if (orderColumnBx.getModel().getElementAt(i).equalsIgnoreCase(column)) {
				orderColumnBx.setSelectedIndex(i);
				break;
			}
		}
		
		orderDirection.setSelected(reverse);
	}
...
//	    if (columnToColor.getModel().getSize()>0) return;
	    ...
	    validNames.remove("Calculated Value");
	    ...
	    if (mySettings.orderColumn!=null && mySettings.orderColumn.length()>0) {
	    	defaultOrderColumn = mySettings.colorColumn;
	    } else if (defaultColumn!=null && defaultColumn.length()>0) { 
	    ...
	    if (orderColumnBx!=null) {
	    	orderColumnBx.setModel(new DefaultComboBoxModel(validNames.toArray()));
	    }
	    ...
	    if (defaultColumn.length()>0) {
	    	...
	    	if (orderColumnBx!=null) {
	    		orderColumnBx.setSelectedItem(defaultColumn);
	    	}
...
//	    columnBx.setModel(new DefaultComboBoxModel(validNames.toArray()));
	    ...
	    if (mySettings.expressionSelected) {
			...
			try {
				String originalText = mySettings.expressionText;
	...
	public int getOrderColumn() {
		if (orderColumnBx==null) return -1;

		String columnName = (String) orderColumnBx.getSelectedItem();
	...
	public boolean getOrderDirection() {
		return orderDirection.isSelected();
	}
	...
	class SaveAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e){
	        ...
	        if (delimiter.equalsIgnoreCase(",")) {
	        	fileChooser.setFileFilter(csvFormatFilter);
	        } else {
...
//	            		}
	                ...
	                for (int i=0; i<rows; i++) {
	                	if (saveGeometry) {
	                		// The nth element in each table row is the actual stamp itself.  Never actually displayed to the user
...
//		                	}
	...
	private void refreshViews() {
		...
		if(view3d.isEnabled()){
			...
			if(mgr.isReady() && view3d.isVisible()){
