					...
					if (e.getButton()==MouseEvent.BUTTON1) {
						if (mode == ADD_MODE) {
							...
							if (settings.filterMainView && !meetsPointFilter) {
								JOptionPane.showMessageDialog(fp, "You are trying to add a point outside your filtered region", "Outside of current crater filter", JOptionPane.ERROR_MESSAGE);
								return;
							}
							...
							if (points.size() == 3) {
								try {
									...
									JOptionPane.showMessageDialog(CraterLView.this, ex.getMessage(), "Crater definition issue", JOptionPane.ERROR_MESSAGE);
								}
...
//	 */
	...
	synchronized void drawSelectedCraters() {
		clearOffScreen(1);
...
//		g2=viewman.wrapScreenGraphics(g2);
	...
	private void selectCraters(List<Crater> cratersToSelect) {
		...
		selectedCraters.addAll(cratersToSelect);		
