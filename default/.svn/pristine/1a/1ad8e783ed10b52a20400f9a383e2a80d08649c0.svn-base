...
public class TestDriverLayered extends JPanel
{
	...
	statusBar.setBorder(BorderFactory.createEmptyBorder(8, 8, 0, 5));
	...
	bottomRow = new JPanel(new GridBagLayout());
	bottomRow.setPreferredSize(new Dimension(this.getPreferredSize().width, 32));
	...
	public void buildViews()
	{
		...
		if (Main.savedLayers != null) {
		    ...
		    for(LViewFactory cartographyFactory : LViewFactory.cartographyList) {
                Layer.LView view = cartographyFactory.showCartographyLView();
                if (view != null) {
