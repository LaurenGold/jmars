...
public class CraterFocusPanel extends FocusPanel {	private final AtomicReference<CraterSettings> craterSettings = new AtomicReference<CraterSettings>();
	...
	public CraterFocusPanel(final LView parent) {
			...
			public void actionPerformed(ActionEvent e){
				...
				while (true) {
					...
					if (files[0].exists()) {
						...
						JOptionPane.showMessageDialog(CraterFocusPanel.this, "File does not exist", "Unable to import file", JOptionPane.ERROR_MESSAGE);
					}
				}
				...
				if (!errors.isEmpty()) {
					JOptionPane.showMessageDialog((JComponent)e.getSource(), Util.join("\n", errors),
		...
		Action exportCsvAction = new AbstractAction("Export CSV".toUpperCase()) {
			public void actionPerformed(ActionEvent e) {
		...
		Action exportDiamAction = new AbstractAction("Export Craterstats".toUpperCase()) {
			public void actionPerformed(ActionEvent e) {
				...
				int option = JOptionPane.showConfirmDialog(null, message, "Enter Craterstats Info", JOptionPane.OK_CANCEL_OPTION);
		...
		JButton bExportCsv = new JButton(exportCsvAction);
		
		JButton bConfig = new JButton(configAction);
		
		JButton bExportDiam = new JButton(exportDiamAction);
		...
		buttonPanel.add(bExportCsv);
		buttonPanel.add(bExportDiam);
...
//		parent.getChild().repaint();
	...
	private JPanel createColorPanel() {
		...
		panel.setBackground(UIManager.getColor("TabbedPane.selected"));
...
//						System.out.println("NOT A VALID VALUE " + command);
		...
		noteLabel2.setBackground(panelBackground);
	    ...
	    noteLabel3.setBackground(panelBackground);
		...
		noteLabel4.setBackground(panelBackground);
	    ...
	    noteLabel5.setBackground(panelBackground);
	...
	private JPanel createDisplaySettingsPanel() {
		...
		filter3dView.setSelected(layer.settings.filterPanView);
		...
		SpringLayoutUtilities.makeCompactGrid(filterPanel, 4, 1, 6, 6, 6, 6); 
		panel.add(filterPanel);
