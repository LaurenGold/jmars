...
public class SpectraView extends JPanel {
	...
	private void buildUI(){
		...
		columnPnl.setBackground(Util.lightBlue);
			...
			if(SwingUtilities.isRightMouseButton(e)){
				...
				JMenuItem renameItem = new JMenuItem(new AbstractAction("Rename spectrum...") {
					public void actionPerformed(ActionEvent e) {
						...
						String name = JOptionPane.showInputDialog(spectraTbl, message, null);
				...
				JMenuItem subtractItem = new JMenuItem(new AbstractAction("Subtract spectra...") {
					public void actionPerformed(ActionEvent e) {
						...
						String firstSpectra = (String) JOptionPane.showInputDialog(spectraTbl,
				...
				JMenuItem pointNormItem = new JMenuItem(new AbstractAction("Point normalize spectra...") {
					public void actionPerformed(ActionEvent e) {
						...
						int result = JOptionPane.showConfirmDialog(spectraTbl, inputPnl,
					...
					public void actionPerformed(ActionEvent e) {
						...
						if(result == JOptionPane.OK_OPTION){
							...
							try{
								...
								JOptionPane.showMessageDialog(spectraTbl, 
										"One or more entries was an invalid number.\n"
	...
	private void setRangeValues(){
		try{
			...
			JOptionPane.showMessageDialog(controlFrame,
					"Invalid inputs. Inputs must be numbers.",
...
//								System.out.println(xVals[i]);
	...
	public void refreshSpectraData(boolean refreshTable){
		...
		if(allObjs.size()>0){
			...
			for(SpectraObject so : allObjs){
				...
				for(int i=0; i<so.getYValues().length; i++){
					data.add(so.getXValues()[i], so.getYValues()[i]);
				}
	...
	public void addSelectionsFromLView(){
		...
		for(StampShape ss: stampLView.stampLayer.getSelectedStamps()){
			for(int i = 0; i<model.getRowCount(); i++){
				if(model.getSpectra(i).getName().contains(ss.getId())){
					rows.add(i);
					break;
				}
			}
	...
	public void showInFrame(){
		...
		if(myFrame == null){
			...
			myFrame.setLocationRelativeTo(stampLView.getFocusPanel());
			myFrame.pack();
