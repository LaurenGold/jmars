	...
	private AbstractAction setInputsAct = new AbstractAction("Set Inputs for Selected Data Point".toUpperCase()) {
		public void actionPerformed(ActionEvent e) {
			...
			if(!check.equals("")){
				...
				JOptionPane.showMessageDialog(MCDFocusPanel.this, errorText, "Invalid Input(s)", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			Thread manager = new Thread(new Runnable() {
				public void run() {
					...
					for(int row : rows){
						...
						if(dp.getHeightData() == null || dp.getLsData() == null || dp.getLocalTimeData() == null){
							System.err.println("Running MCD for "+dp.getName());
							runMCD(dp);
						}
					}
					...
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							...
							JOptionPane.showMessageDialog(MCDFocusPanel.this, "MCD Run Complete!", "Successful MCD Run", JOptionPane.INFORMATION_MESSAGE);
						}
					});
				}
			});
...
//		urlStr += "&process="+MCDLayer.POINT_MODE_PROCESS+"&version="+MCDLayer.SCRIPT_VERSION;
		...
		try{
			...
			ArrayList<Double> heightX = values.get(0);
			ArrayList<Double> heightY = values.get(1);
			ArrayList<Double> lsX = values.get(2);
			ArrayList<Double> lsY = values.get(3);
			ArrayList<Double> ltX = values.get(4);
			ArrayList<Double> ltY = values.get(5);
			
			XYSeries heightSeries = new XYSeries(dp.getName());
	        for (int i=0; i<heightX.size(); i++) {
	            heightSeries.add(heightX.get(i), heightY.get(i));
	        }
	        
	        XYSeries lsSeries = new XYSeries(dp.getName());
	        for(int i=0; i<lsX.size(); i++){
	        	lsSeries.add(lsX.get(i), lsY.get(i));
	        }
	        
	        XYSeries ltSeries = new XYSeries(dp.getName());
	        for(int i=0; i<ltX.size(); i++){
	        	ltSeries.add(ltX.get(i), ltY.get(i));
	        }
	        
	        dp.setHeightData(heightSeries);
	        dp.setLsData(lsSeries);
	        dp.setLocalTimeData(ltSeries);
	...
	private JTable createReadoutTable(int dataType){
		...
		ReadoutTableModel tm = new ReadoutTableModel(this, tableModelType, data, chart);
	...
	private void refreshCharts(){
		...
		for(int row : resultsDataTbl.getSelectedRows()){
			MCDDataPoint dp = ((DataTableModel)resultsDataTbl.getModel()).getDataPoint(row);
			if(dp.getHeightData()!=null){
				XYSeries data = dp.getHeightData();
				heightData.addSeries(data);
			}
			if(dp.getLocalTimeData()!=null){
				ltData.addSeries(dp.getLocalTimeData());
			}
			if(dp.getLsData()!=null){
				lsData.addSeries(dp.getLsData());
			}
		}
	...
	private JFreeChart createChart(XYSeriesCollection dataset, int dataType) {
		...
		//use day values
		if(dataType == HEIGHT_DATA){
			title = "Temperature at Height Above Surface";
			range = heightRange;
	...
	private void refreshInputs(){
		...
		//enable and populate the inputs based off the selected data point
			...
			if(statFailures.size()>0){
				...
				JOptionPane.showMessageDialog(MCDFocusPanel.this, errorText, "Default(s) not found", JOptionPane.ERROR_MESSAGE);
			}
