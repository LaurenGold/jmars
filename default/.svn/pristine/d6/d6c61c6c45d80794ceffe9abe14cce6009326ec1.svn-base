...
public void sendMesh(Mesh m){
  String[] ipaddresses = settings.ipaddress.split(",");
  if (ipaddresses.length ==0){return;}
    try{
        
        ByteBuffer bb = ByteBuffer.allocate(m.vertices.length*4+8); //4 bytes per vertex float + 4 bytes for width + 4 bytes for height
        ...
        FileOutputStream fos = new FileOutputStream("mesh.jmars");
        fos.write(bb.array());
        fos.close();
...
public void sendTexture(byte[] byteArray){
  ...
  String[] ipaddresses = settings.ipaddress.split(",");
  try{
    FileOutputStream fos = new FileOutputStream("mesh.jmars.png");
        fos.write(byteArray);
        fos.close();
  }catch(Exception e){
    System.err.println(e);
  ...
  for (int ip_index=0; ip_index<ipaddresses.length; ip_index++){
	...
	public BufferedImage getDepthImage() {
		...
		final BufferedImage image = new BufferedImage(elevation.getWidth(), elevation.getHeight(), BufferedImage.TYPE_USHORT_GRAY);
		final Function<Float, Integer> convertToGrayScale = z -> new Double((z - zMin) / zRange * USHORT_MAX).intValue();
		final Function<Float[], Stream<Integer>> rasterizeRow = row -> Arrays.stream(row).map(convertToGrayScale);

		int[] pixels = Arrays.stream(elevations)
				.map(ArrayUtils::toObject)
				.flatMap(rasterizeRow)
