				...
				public void run() {
					...
					try{
						...
						if(myLayer.getElevationSource() == null){
							...
							FieldMap elevMap = new FieldMap("map sampling", Type.AVG, ppd, myLayer.getElevationSource(), 0);
						...
						if(myLayer.getAlbedoSource() == null){
							...
							FieldMap albedoMap = new FieldMap("map sampling", Type.AVG, ppd, myLayer.getAlbedoSource(), 0);
						...
						if(myLayer.getThermalInertiaSource() == null){
							...
							FieldMap tiMap = new FieldMap("map sampling", Type.AVG, ppd, myLayer.getThermalInertiaSource(), 0);
						...
						if(myLayer.getSlopeSource() == null){
							...
							FieldMap slopeMap = new FieldMap("map sampling", Type.AVG, ppd, myLayer.getSlopeSource(), 0);
						...
						if(myLayer.getAzimuthSource() == null){
							...
							FieldMap azimuthMap = new FieldMap("map sampling", Type.AVG, ppd, myLayer.getAzimuthSource(), 0);
							azimuth = getSampledValue(azimuthMap, point);
						}
						//if the source was null then sampling couldn't be done
						if(nullSource){
