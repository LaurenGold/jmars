...
public class FocusPanel extends JTabbedPane
{
	...
	public FocusPanel(Layer.LView parent, boolean isLimited){
		...
		if(parent==null){
			...
			parentFrame.setTitle(parent.getName() + FOCUS_TITLE);
		}
	    ...
	    //Material
	    this.setOpaque(true);
	    Color background = ((ThemePanel)GUITheme.get("panel")).getBackground();
    ...
    public void addTab(String title, Component component) {
    	...
    	if (dockLocation == -1)
    		super.addTab(title.toUpperCase(), component);
    		 ...
    		 insertTab(title.toUpperCase(), null, component, null, dockLocation); 
    	}
    ...
    public void restoreInFrame(int locX, int locY, int width, int height) {
    	this.addTab("Dock Me".toUpperCase(), dockMe);
    ...
    {        
        ...
        preferred.width = Math.max(preferred.width, 900);
        return preferred;
    }
