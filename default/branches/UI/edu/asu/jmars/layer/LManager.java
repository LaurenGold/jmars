...
public class LManager extends JPanel {
    ...
    public void dockTab(String name, Icon icon, Component fp, boolean selectTab) {
		dockTabs.addTab(name.toUpperCase(), icon, fp);
		if (selectTab) {
	...
	public void deleteSelectedLayer() {
		...
		if (mgr.isReady() && view3d != null && view3d.exists()){
	...
	*/
	...
	public void accessSelectedOptions(boolean openDocked) {
		...
		Component c = LManager.getLManager().getMainPanel().getAddLayerDialog();
	...
	public void addSavedLView(Layer.LView view) {
        ...
        //Need to take the cartography layers (always on top) into account
        if (isOverlay(view)) {
            if (!checkOverlayTypeLoaded(view.getClass())) {
                //we do not have one of these loaded in the overlay section yet
                viewList.add(view);
                overlayFlag = true;
            }
        }
