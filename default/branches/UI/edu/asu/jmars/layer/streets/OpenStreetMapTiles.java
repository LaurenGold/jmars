	  ...
	  public void getTileImages(BoundingBoxDataObject boundingBox, int zoom, Double jmarsPpdDouble, double equaCircumference) {  
		  ...
		  try {
			...
			// pop up informational dialog to the user
			  JOptionPane.showMessageDialog(null, "Can't save to JMARS cache folder. Please check to make sure there is enough " +
...
//				  img = ImageIO.read(url);
				  ...
				  if (e instanceof IOException){
					  ...
					  JOptionPane.showMessageDialog(null, "Error, the OpenStreetMap data is " +
					  ...
					  JOptionPane.showMessageDialog(null, "Error: Serious system error, please submit a problem report " +
		  ...
		  // In the future, we may change osm to draw pixel to pixel instead of using bufferedImages
			...
			for (LView lView : al) {
				if (lView instanceof StreetLView) {
					...
					if (myLView.isAlive()){
						JOptionPane.showMessageDialog(null, "Sorry, OpenStreetMap does not re-project at this time.","Message", JOptionPane.ERROR_MESSAGE);				
