	...
	public BufferedImage createDataImage(Rectangle2D worldRect, ProjObj po, int ppd, int labelScaleFactor){
		...
		for(MarsFeature mf : myLView.landmarks){
			if(settings.showLandmarkTypes.contains(mf.landmarkType)){
				...
				//some of the decals start past 360, so adjust the world
				// point before checking to see if it intersects the decal
				if(worldRect.getX()>360){
					worldPt = new Point2D.Double(worldPt.getX()+360, worldPt.getY());
				}
				
				//check if landmark is within decal bounds
				if(worldRect.contains(worldPt)){
