...
public class NorthLView3D extends LView3D{
	...
	public BufferedImage createDataImage(Rectangle2D worldRect, ProjObj po, int ppd, int labelScaleFactor){
		...
		Shape shape = arrow.getArrowForScale(labelScaleFactor);
		...
		int xOffset = arrow.getXOffset()+(20*(labelScaleFactor-1));
		int yOffset = arrow.getYOffset()+(20*(labelScaleFactor-1));
		...
		g2.setStroke(new BasicStroke(settings.outlineSize*labelScaleFactor));
		...
		if(settings.showText){
			...
			g2.setFont(arrow.getTextFont(settings.fontSize * labelScaleFactor));
