...
public class HorizonPanel extends JPanel{
	...
	private void buildLayout(){
		...
		widthPnl = new JPanel();
		widthPnl.setBackground(Util.lightBlue);
		...
		editPnl.setBackground(Util.lightBlue);
	...
	private AbstractAction delAct = new AbstractAction("Delete Selected".toUpperCase()) {
		public void actionPerformed(ActionEvent e) {
			...
			int result = JOptionPane.showConfirmDialog(delBtn, 
	...
	private AbstractAction csvAct = new AbstractAction("Export to CSV".toUpperCase()) {
		public void actionPerformed(ActionEvent e) {
			...
			RadarHorizon h = getSelectedHorizon();
			JFileChooser fc = new JFileChooser(Main.getUserHome());
			...
			if(fc.showSaveDialog(HorizonPanel.this) == JFileChooser.APPROVE_OPTION){
				...
				if(succeed){
					JOptionPane.showMessageDialog(HorizonPanel.this, "Export Successful!", "Export Success", JOptionPane.INFORMATION_MESSAGE);
				}else{
					JOptionPane.showMessageDialog(HorizonPanel.this, "Export Not Successful.\nSee log for more info.", "Export Failure", JOptionPane.INFORMATION_MESSAGE);
				}				
			}
	...
	private void setEnabledEditPanel(boolean enable){
		...
		if(enable){
			editPnl.setBackground(Util.lightBlue);
			widthPnl.setBackground(Util.lightBlue);
			diffPnl.setBackground(Util.lightBlue);
			dBtnPnl.setBackground(Util.lightBlue);
		}else{
			editPnl.setBackground(Util.panelGrey);
			widthPnl.setBackground(Util.panelGrey);
			diffPnl.setBackground(Util.panelGrey);
			dBtnPnl.setBackground(Util.panelGrey);
		}
