...
//		    			Rectangle2D rect = new Rectangle2D.Double(minExtent.getX(), maxExtent.getY(), width, height);
		    			...
		    			if(layer.vectorShapes() || layer.pointShapes() || layer.spectraData()) {
			    			...
			    			OutlineFocusPanel ofp = layer.viewToUpdate.myFocus.outlinePanel;
	...
	//								}
				    			...
				    			// TODO: This should probably be passed in as a layer parameter
								double degrees = 0.00254;  // Approximate number of degrees in the diameter of a 150 meter spot
								degrees = degrees / 2.0;				
	...
	//								}
									...
									Point2D  p=  ((PointShape)stamp).getOrigin();
									
				    				double val = Double.parseDouble(stamp.getStamp().getData()[columnToColor]+"");
...
//				    				g2.setPaint(color);
							    	...
							    	if (bp!=null) {
								    	Point2D bw = po.convSpatialToWorld(bp);
								    	Color contrastColor = new Color(StampUtil.getContrastVersionForColor(color));
								    	g2.setPaint(contrastColor);
								    	
								    	g2.fillOval((int)Math.round(bw.getX()), (int)Math.round(bw.getY()), 2, 2);
							    	} 
				    			}											    			
							}
...
//							    	
						    	...
						    	if (bp!=null) {
							    	...
							    	Color contrastColor = new Color(StampUtil.getContrastVersionForColor(unsColor));
							    	g2.setPaint(contrastColor);
...
//		
	...
	public static BufferedImage getSelectionsImage(StampLayer stampLayer, final List<StampShape> selectedStamps, Rectangle2D worldRect, ProjObj po, int ppd, int scale, int stateId, boolean checkId){
		...
		Color inverseColor = new Color(StampUtil.getContrastVersionForColor(stampLayer.getSettings().getUnselectedStampColor()));
...
//				System.out.println("adot = " + dot + "    " + p1.lon() + " : " + p1.lat() + "   " + p2.lon() + " : " + p2.lat());
	...
	//			}
		    	...
		    	fillUp = isPointInsidePolygon(spatialPts, spatialNorth, crossesMeridian);			
			}
		}
     ...
     * @return
     */
    private static List<Area> clippedArea(Area fill, Area a1Area) {
    ...
    public static Analysis analyzePoints(double pts[], ProjObj po) {
    	// TODO: If the polygon is not explicitly closed, this routine can miss the final segment back to close the polygon
