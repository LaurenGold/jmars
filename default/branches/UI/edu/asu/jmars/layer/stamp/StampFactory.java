...
//		} catch (UnknownHostException uhe) {
		...
		if (this.isStampServerAvailable()) {
			if (instrument.equalsIgnoreCase("davinci")) {
				...
				if (!dialog.isCancelled()) {				
					...
					StampLView view = new StampLView(StampFactory.this, layer, null, layerParams, new StampLView3D(layer));
				...
				AddLayerDialog dialog = AddLayerDialog.getStampLayerDialog(wrapper, true);
				...
				if (!dialog.isCancelled()) {
					...
					StampLayer layer = new StampLayer(newSettings);
					StampLView view = new StampLView(StampFactory.this, layer, wrapper, layerParams, new StampLView3D(layer));
					layer.setViewToUpdate(view);
    ...
    public Layer.LView recreateLView(SerializedParameters parmBlock) {
            ...
            parmBlock instanceof StampLayerSettings)
        {
            ...
            view = new StampLView(this, stampLayer, null, settings.getLayerParams(), new StampLView3D(stampLayer));
    ...
    public static void createOverlappingStampLayer(String type, ArrayList<String> ids, ArrayList<GeneralPath> paths) {
		...
		AddLayerDialog dialog = AddLayerDialog.getStampLayerDialog(wrapper,false);
		dialog.setVisible(true);
		if (!dialog.isCancelled()) {
			StampLayerSettings newSettings = new StampLayerSettings();
			if (dialog.initialName.getText().trim().length()>0) {
			...
			StampLayer layer = new StampLayer(newSettings);
			StampLView view = new StampLView(new StampFactory(), layer, wrapper, null, new StampLView3D(layer));
			layer.setViewToUpdate(view);
